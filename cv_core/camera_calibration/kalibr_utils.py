...
class KalibrCamChain:
    def __init__(self, camchain_file=None):
        self.data = None
        if camchain_file is not None:
            self.load(camchain_file)

    def load(self, camchain_file, replace=True):
        """
        load camchain file
        you can load a new camchain file even if you already loaded one!

        :param camchain_file: file path
        :param replace: if existing data overlaps with the new loaded data:
                        - replace=True means new data replaced old data
                        - replace=False means new data is ignored
                        replacing is done at the camera block level, and not for separate fields inside eac camera block
        :return:
        """
        if not os.path.isfile(camchain_file):
            Exception('kalibr camchain file: {} not found!'.format(camchain_file))
        with open(camchain_file, 'r') as file:
            self.data = yaml.safe_load(file)
        return

    def save(self, camchain_file):
        """
        save camchain file
        :param camchain_file: file path
        :return:
        """
        folder_name = os.path.dirname(camchain_file)
        if not os.path.isdir(folder_name):
            os.makedirs(folder_name)
        if not os.path.isfile(camchain_file):
            os.remove(camchain_file)
        with open(camchain_file, 'w') as outfile:
            yaml.dump(self.data, outfile, default_flow_style=None, sort_keys=False)
        return
