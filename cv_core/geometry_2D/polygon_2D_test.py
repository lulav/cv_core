...
import cv_core.geometry_2D.line_2D as l2di
...
def intersect_image_borders_rectangle(draw=False):
    ...
    for i in range(points.shape[0]):
        ip, n = l2di.polygon_segment_intersection(poly_points, center_point, points[i, :], epsilon=1e-8)
...
def line_segments_intersect_test(draw=False):
...
def line_polygon_intersect_test(draw=False):
...
if __name__ == "__main__":
    ...
    res = line_segments_intersect_test(draw=draw)
    if res:
